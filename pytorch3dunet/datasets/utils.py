...
def get_train_loaders(config: dict) -> dict[str, DataLoader]:
    ...
    train_config = loaders_config['train']
    logger.info(f"Train config: {train_config}")

    # 如果 val 未指定，则从 train 复制必要配置
    if 'val' not in loaders_config:
        val_config = {
            'file_paths': train_config['file_paths'],
            'shuffle': False,
            'transformer': train_config.get('transformer', {}),
            'slice_builder': train_config.get('slice_builder', {})
        }
        loaders_config['val'] = val_config
        logger.info("Validation config not specified, using train config with validation_split.")

    # 获取 validation_split
    validation_split = config.get('trainer', {}).get('validation_split', 0.0)
    if validation_split > 0:
        # 处理 file_paths 列表，展开通配符
        file_paths = []
        for path in train_config['file_paths']:
            file_paths.extend(glob.glob(path))  # 展开 './data/train/*.h5'
        if not file_paths:
            raise ValueError(f"No files found matching {train_config['file_paths']}")

        # 创建完整数据集列表
        full_datasets = []
        for file_path in file_paths:
            dataset = dataset_class(file_path,
                                  transformer_config=train_config.get('transformer', {}),
                                  slice_builder_config=train_config.get('slice_builder', {}),
                                  phase='train')
        ...
        split_idx = int(total_size * (1 - validation_split))
        
        # 分割数据集
        train_datasets = torch.utils.data.Subset(full_dataset, range(0, split_idx))
        val_datasets = torch.utils.data.Subset(full_dataset, range(split_idx, total_size))
        
        logger.info(f"Total dataset size: {total_size} patches")
        logger.info(f"Training dataset size: {len(train_datasets)} patches")
        logger.info(f"Validation dataset size: {len(val_datasets)} patches")
    else:
    ...
    # 从 train_config 中读取 batch_size 和 num_workers
    ...
    return {
        ...
        'val': DataLoader(val_datasets, batch_size=batch_size, shuffle=False, pin_memory=True,
                          num_workers=num_workers, drop_last=True)
    }

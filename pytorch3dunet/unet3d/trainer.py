...
class UNetTrainer:
    ...
    def train(self):
        ...
        for i, t in enumerate(self.loaders['train'], 1):
            ...
            # 清理内存
            del input, target, output, loss  # 删除引用
            if str(device) == 'mps':  # 从模型参数获取设备                
                torch.mps.empty_cache()  # 清理 MPS 缓存，每迭代清理一次
            # 验证
            ...
            if self.num_iterations % self.log_after_iters == 0:
                ...
                logger.info(f'Training stats. Loss: {train_losses.avg:.4f}. Evaluation score: {train_eval_scores.avg:.4f}')
                self._log_stats('train', train_losses.avg, train_eval_scores.avg)
                
                # 减少图像日志频率
                if self.num_iterations % (self.log_after_iters * 10) == 0:  # 每 10 次记录图像
                    with torch.no_grad():
                        input, target = _split_and_move_to_gpu(t)
                        output = self.model(input)
                        self._log_images(
